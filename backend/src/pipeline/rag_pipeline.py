...
def get_llm_client():
    if GROQ_API_KEY and GROQ_API_KEY.strip() not in ["your_groq_api_key_here", ""]:
    ...
    elif AZURE_OPENAI_API_KEY and AZURE_OPENAI_ENDPOINT and "your_key" not in AZURE_OPENAI_API_KEY:
        return AzureOpenAI(
        ...
        raise ValueError(
            "No valid API key configured! Please open 'backend/.env' and replace "
            "'your_groq_api_key_here' with your real Groq API key (starts with 'gsk_')."
        )
